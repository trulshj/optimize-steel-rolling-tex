...
We start by finding the weak formulation for the problem defined in \eqref{eq:heat}. We multiply the equation \eqref{eq:heat-in-omega} by a test function $\psi\in H^1(\Omega)$, and integrate over the domain $Q$. Assuming that $k$ is a scalar, partial integration gives
\begin{equation}
\begin{aligned}
...
\begin{subequations}
   ...
   \end{align}
\end{subequations}
This adjoint equation for the adjoint state $p$ runs backwards in time, but since the final condition i.e. $p(x,T)$ is predescribed instead of the initial condition, the problem is well-posed. If one had posed $p|_{t=0}$ instead one would have gotten an ill-posed backward parabolic equation. driving force is end termperature differnce

\subsubsection{Gradient}
To derive the gradient of the cost functional given in \eqref{eq:cost-func}, we differentiate the Lagrangian \eqref{eq:lagrangian-raw} again, but now with respect to the control $u$. We still differentiate in direction $h$, but now note that $h\in H^1(0, T)$. This gives
\begin{equation}
\begin{aligned} % Fikk hjelp av Dietmar for denne, så den skal være good.
  \L_u(\theta, u, p) h &= \gamma\int_0^T uh \dt - \frac{1}{\rho c_p} \iint\limits_{0\,\,\Gamma_1}^{\,\,\,T}(\theta - \theta_w)p \dsdt \\
  &= \int_0^T h \left( \gamma u - \frac{1}{\rho c_p} \int_{\Gamma_1}(\theta - \theta_w)p \mathop{ds} \right) \dt.
\end{aligned}
\end{equation}

The space $H^1(0,T)$ is a hilbert space. By Riesz representation theorem we can identify $H$ with $H^{*}$ for any Hilbert space $H$. Therefore by identifying using an isomorphism we find that the u-derivative of the lagranginan is

\begin{equation*}
    \L_u(\theta,u,p) = \gamma u - \frac{1}{\rho c_p}\int_{\Gamma_1}(\theta - \theta_w)p \ds
\end{equation*}
...
Each of these are first-order optimality condition which needs to be satisfied for a control $u\in U_{ad}$. We know that if the control-to-state operator $e(y,u) = 0$ representing the heat equation is bijective then $\exists$ an adjoint state $p\in (range(e(y,u))^{*}$ such that the optimaility system is satisfied. Now one can restate this optimalilty system using the projection formula if one assume box-constraints. That is if we assume
