...
\begin{subequations}
   ...
   \end{align}
\end{subequations}
Where $u = u(t) \colon \mathbb{R} \to \mathbb{R}$ is the scalar-valued, time dependent heat transfer coefficient which acts as our unknown control. $c_p$ is the heat capacity, $k$ is thermal conductivity and $\rho$ is the density. The Neumann boundary condition in (1b) is based on Newton's law of cooling, here $\theta_w$ is the temperature of the applied coolant at the boundary. The goal is to determine a optimal $u$ such that $\theta(x, T)$ is "as close as possible" to some desired temperature $\theta_d$, where $T$ denotes the final time of the process, in order to model this we introduce a quadratic tracking term into the cost functional. Furthermore we require the control $u$ to be admissible. More formally, we wish to minimize a cost functional of the form
\begin{subequations}
